...
async function getUserIdByMobile(mobile) {
  ...
  try {
    const response = await axios.get(
      `${config.feishu.apiBase}/open-apis/contact/v3/users/find_by_mobile`,
      {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
    ...
    console.log(`[Feishu] 通过手机号获取用户失败：${error.message}`);
    return null;
  }
}
 ...
 * 发送群消息文本
 */
async function sendGroupTextMessage(chatId, text) {
  const token = await getTenantAccessToken();

  console.log('[Feishu] 发送群消息:', {
    chatId,
    text: text.substring(0, 50),
    apiBase: config.feishu.apiBase
  });

  try {
    // 飞书 API：使用 receive_id_type=chat_id 的方式
    const response = await axios.post(
      `${config.feishu.apiBase}/open-apis/im/v1/messages?receive_id_type=chat_id`,
      {
        receive_id: chatId,
        msg_type: 'text',
        content: JSON.stringify({ text })
    ...
    console.log('[Feishu] 群消息发送成功:', response.data);
    return response.data.data;
  } catch (error) {
    console.error('[Feishu] 群消息发送失败:', {
...
export default {
  ...
  createCoachCard,
  createRiskAlertCard,
  generateQuestionsViaOpenClaw,
  generateQuestionsWithDashScope
};
